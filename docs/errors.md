...
| `helper-unavailable` | An explicitly required addon or necessary OS primitive is unavailable. | Global `FS_SAFE_NATIVE_MODE=require` with an omitted/missing/incompatible addon, or a genuinely unavailable filesystem primitive. Normal `auto` and `off` use portable feature implementations when the addon is absent. |
...
| `unsupported-platform` | The platform or filesystem cannot perform the requested operation. | Native cloning absence selects portable copying, including a warning for `clone: "always"`; unsupported source contents or metadata still fail. Other examples include requesting Windows ACL facts on a different OS or lacking a required filesystem operation. See [directory copying](copy.md) for backend limits and fallback behavior. |
