...
- Verify async and sync regular-file append targets with exact bigint preview, descriptor, and current-path identities; reject rounded-equal replacements and persistent unknown Windows identities before chmod or append.
...
- Add descriptor-bounded temp workspace cleanup with collision-safe quarantine and native handle-relative tree removal; prevent recursive traversal of raced replacement trees, bind enumerated children before descent, reject mount crossings, remove symlink/reparse entries as leaves without traversing their targets, bound POSIX's irreducible name-based unlink race to one leaf or empty directory entry, delete exact opened objects on Windows, and require the exact Linux `openat2`/`RESOLVE_NO_XDEV` capability at runtime: compatible cleanup falls back when unavailable, while `cleanupSafety: "require-bounded"` rejects before child creation.
