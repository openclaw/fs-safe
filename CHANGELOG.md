...
- Add opt-in `producerIsolation: "private-directory"` to callback sibling writes and external sibling outputs, owning private workspace cleanup before the producer runs so partial failures can be cleaned while preserving file admission, publication checks, and existing defaults. Reuse the already admitted parent guard when constructing the private producer Root instead of repeating canonicalization and identity capture.
