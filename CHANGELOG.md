...
- Bind ZIP entry kinds to physical metadata before filters, extraction, or bounded reads. Preserve terminal directory markers across local, central, and Unicode names; recognize native DOS/UNIX directory attributes with symlink precedence; reject unsupported or disagreeing portable decoder kinds with `ArchiveFormatError`.
