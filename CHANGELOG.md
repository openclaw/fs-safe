...
- Verify `Root.write()` and `create()` through retained descriptors and exact bigint identities so final modes such as `000` and large file indexes succeed without relaxing permissions or post-publication checks; preserve explicit mode zero in `copyIn()`.
