...
- Keep `Root.stat()` and `Root.list()` descendant observations inside the admitted root by checking exact target, parent, and selected-directory identities before returning metadata or names.
