...
- Keep `Root.stat()` and `Root.list()` descendant observations inside the admitted root with exact target, parent, and selected-directory checks; retain the admitted parent when initial target lookup fails, and fuse directory identity and canonical-path observations through one no-follow native handle when available, preserving the guarded JavaScript fallback before admission.
