...
- Make no-clobber `Root.move()` use the native descriptor-relative no-replace rename after admitting and rechecking both parent directories. A concurrent target is now preserved with `already-exists`; unavailable native admission fails closed with `helper-unavailable` instead of using a check followed by a replacing rename. `{ overwrite: true }` behavior is unchanged.
- Preserve native-off isolated sibling producers with an identity-fenced, no-clobber hard-link handoff. Windows avoids completed-file data-read admission overhead through a write-only descriptor with a bounded historical-access fallback. Public no-clobber `Root.move()` remains native-only; unsupported hard-link filesystems fail explicitly instead of copying or replacing a competing stage.
