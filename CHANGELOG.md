...
- Add descriptor-bounded temp workspace cleanup with collision-safe quarantine and native handle-relative tree removal; prevent recursive traversal of raced replacement trees, bind enumerated children before descent, reject mount crossings, bound POSIX's irreducible name-based unlink race to one leaf or empty directory entry, delete exact opened objects on Windows, retain the documented native-off compatible quarantine fallback, and expose `cleanupSafety: "require-bounded"` for callers that must reject that fallback before child creation.
