...
- Bind secure-temp mode repair and created-directory finalization to exact owned no-follow descriptors, removing pathname chmod and rejecting observed replacements; require a complete explicit descriptor adapter for injected filesystems, retain the deprecated unused chmod hook, and preserve the secure existing-directory fast path plus Windows type/access admission.
