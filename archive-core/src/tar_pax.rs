...
impl LocalPax {
    // The meter already admitted raw fields, safe size, and zero-sized directory/link bodies.
    pub fn member_size(&self, entry_type: u8, raw_size: u64, raw_name: &[u8]) -> io::Result<u64> {
        ...
        if entry_type != b'5' && (self.path_trailing_separator || raw_name.last() == Some(&b'\\')) {
            return Err(invalid());
        }
        if self.linkpath && !matches!(entry_type, b'1' | b'2') {
            return Err(invalid());
        }
        Ok(size)
    }
