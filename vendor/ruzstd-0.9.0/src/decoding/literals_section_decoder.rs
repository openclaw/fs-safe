...
fn decompress_literals(
    ...
    let stream_sizes = if num_streams == 4 {
        let first = regenerated_size.div_ceil(4);
        let fourth = regenerated_size.checked_sub(first * 3).ok_or(
            err::DecodedLiteralCountMismatch {
                decoded: first * 3,
                expected: regenerated_size,
            },
        )?;
        [first, first, first, fourth]
    } else {
        [regenerated_size, 0, 0, 0]
    };
    target.reserve(regenerated_size);
    ...
    if num_streams == 4 {
        ...
        for (stream, expected) in [stream1, stream2, stream3, stream4].iter().zip(stream_sizes) {
            ...
            // Each stream owns its declared output segment; stop before growing past it.
            while target.len() < end
                && br.bits_remaining() > -(scratch.table.max_num_bits as isize)
            {
        ...
        decoder.init_state(&mut br);
        while target.len() < regenerated_size
            && br.bits_remaining() > -(scratch.table.max_num_bits as isize)
        {
            target.push(decoder.decode_symbol());
            decoder.next_state(&mut br);
