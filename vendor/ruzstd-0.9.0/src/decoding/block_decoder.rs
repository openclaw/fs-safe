...
fn check_block_size(size: u64, limit: usize) -> Result<(), DecompressBlockError> {
    if size > limit as u64 {
        return Err(DecompressBlockError::BlockSizeExceedsLimit { size, limit });
    }
    Ok(())
}
...
impl BlockDecoder {
    pub fn decode_block_content(
        ...
        let block_limit = workspace.buffer.window_size.min(MAX_BLOCK_SIZE as usize);
        let advertised_size = match block_type {
            BlockType::Compressed => header.content_size,
            _ => header.decompressed_size,
        };
        check_block_size(u64::from(advertised_size), block_limit)?;
        match block_type {
            BlockType::RLE => {
                let mut buf = [0u8; 1];
    ...
    fn decompress_block(
        ...
        let block_limit = workspace.buffer.window_size.min(MAX_BLOCK_SIZE as usize);
        // Validate expansion before allocating the regenerated literals.
        check_block_size(u64::from(section.regenerated_size), block_limit)?;
        ...
        // Every sequence has a match length of at least three. Reject an
        // impossible count before decode_sequences reserves its output vector.
        check_block_size(
            u64::from(section.regenerated_size) + u64::from(seq_section.num_sequences) * 3,
            block_limit,
        )?;
        ...
        if seq_section.num_sequences != 0 {
            ...
            // All literals are emitted exactly once, plus every match. Charge
            // the complete block before any sequence extends decoder history.
            let mut remaining = block_limit - workspace.literals_buffer.len();
            for sequence in &workspace.sequences {
                remaining = remaining.checked_sub(sequence.ml as usize).ok_or(
                    DecompressBlockError::BlockSizeExceedsLimit {
                        size: (block_limit - remaining) as u64 + u64::from(sequence.ml),
                        limit: block_limit,
                    },
                )?;
            }
