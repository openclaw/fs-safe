...
function validatePlan(planPath, mode) {
  ...
  assert.equal(plan.experiment, EXPERIMENT);
...
async function spawnChild(configuration) {
  ...
  const result = await new Promise((resolve) => {
    const child = spawn(process.execPath, [process.argv[1], "--experiment", EXPERIMENT, "--child-config", encoded], {
...
async function childMain(encoded) {
  ...
  const receipt = {
    schema: 1, experiment: EXPERIMENT, passed: false, launchOrdinal: config.launchOrdinal,
...
async function orchestratorMain(values) {
  ...
  try {
    ...
    if (mode === "production") assert.deepEqual(sourceTrees, plan.trees, "frozen source tree mismatch");
    ...
    for (const block of schedule) {
      ...
      for (const [slot, processPlan] of block.processOrder.entries()) {
        ...
        for (const key of ["launchOrdinal", "block", "cohort", "slot", "comparison", "label", "revision"]) {
          assert.equal(receipt[key], configuration[key], `child receipt ${key} differs from schedule`);
        }
        assert.equal(receipt.workloadRotation, configuration.rotation);
        assert.deepEqual(receipt.workloads.map(({ id }) => id), plan.workloads.map(({ id }) => id));
        assert.equal(receipt.fixtureManifestSha256, integrityBefore.privateFixtureManifestSha256);
        assert.equal(receipt.nativeLoaderCalls, 0);
        assert.deepEqual(receipt.nativeLoads, []);
    ...
    const report = {
      schema: 1, experiment: EXPERIMENT, mode, notPerformanceEvidence: mode === "smoke", classification,
...
const { values } = parseArgs({
  options: {
    experiment: { type: "string" }, mode: { type: "string" }, plan: { type: "string" },
