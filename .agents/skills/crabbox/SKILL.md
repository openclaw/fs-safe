...
With the repo's pnpm 12, pass flags directly after the script name (for
example, `pnpm crabbox:run --provider aws --id <cbx_id>`). An extra `--`
is forwarded to Crabbox and can turn its options into workload arguments.
Keep Crabbox's own command separator, such as `--shell -- "pnpm test"`.
