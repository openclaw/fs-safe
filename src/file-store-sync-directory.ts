...
function inspectStoreDirectory(params: {
  ...
  return inspectFileIdentitySync(() => {
    const stat = fs.lstatSync(params.dir, { bigint: true });
...
function assertRootCurrent(
  ...
  try {
    if (realpathSync(rootReceipt.dir) !== rootReal) {
      throw changedStoreDirectory(messagePrefix);
    }
  } catch (error) {
    if (error instanceof FsSafeError && error.code === "outside-workspace") throw error;
...
function assertModeTargetCurrent(
  ...
  try {
    targetReal = realpathSync(target.dir);
  } catch (error) {
    throw changedStoreDirectory(messagePrefix, error);
  }
...
function assertReceiptChain(
  ...
  try {
    finalReal = realpathSync(finalReceipt.dir);
  } catch (error) {
    throw changedStoreDirectory(messagePrefix, error);
  }
...
function openStoreDirectoryForMode(params: {
  ...
  try {
    return fs.openSync(params.receipt.dir, directoryOpenFlags());
  } catch (error) {
    openError = error;
  }
  ...
  if (searchFlags !== undefined) {
    try {
      return fs.openSync(params.receipt.dir, searchFlags);
    } catch (searchError) {
...
export function ensureSyncStoreDirectory(params: {
  ...
  fs.mkdirSync(recursiveMkdirPath(rootDir), { recursive: true, mode: params.mode });
  ...
  assertRootCurrent(receipts[0]!, rootReal, params.messagePrefix);
  ...
  for (const segment of relative.split(path.sep).filter(Boolean)) {
    ...
    assertAdmittedEdgeCurrent(receipts, rootReal, params.messagePrefix);
...
export function assertSyncStoreDirectoryReceipt(
  ...
  try {
    ...
    if (stat.dev !== receipt.exactStat.dev || stat.ino !== receipt.exactStat.ino ||
      realpathSync(receipt.dir) !== receipt.realPath) {
