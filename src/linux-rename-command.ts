...
export function isHardlinkCapabilityError(error: unknown): boolean {
  return ["EPERM", "ENOSYS", "ENOTSUP", "EOPNOTSUPP"].includes((error as NodeJS.ErrnoException | undefined)?.code ?? "");
}

export function openLinuxRenameParentSync(parentPath: string, identity: Identity): number {
  // O_PATH accepts write/search-only directories without adding read access.
  const fd = fs.openSync(parentPath, 0x0020_0000 | fs.constants.O_DIRECTORY | fs.constants.O_NOFOLLOW);
  try {
    inspectFileIdentitySync(() => {
      const stat = fs.fstatSync(fd, { bigint: true });
      if (!stat.isDirectory()) throw new FsSafeError("path-mismatch", "rename parent changed");
      return stat;
    }, identity);
    return fd;
  } catch (error) {
    try { fs.closeSync(fd); } catch (closeError) {
      throw createSuppressedError(closeError, error, "rename parent admission and close failed");
    }
    throw error;
  }
}
...
export function renameLinuxNoReplaceSync(input: LinuxRenameNoReplaceInput): void {
  ...
  // Node also reports post-spawn pipe errors here; only an unstarted process
  // proves that one of these admission errors preceded the rename.
  if (result.pid === 0 && result.status === null && result.signal === null && result.error &&
      ["ENOENT", "EACCES", "ENOEXEC"].includes((result.error as NodeJS.ErrnoException).code ?? "")) {
    throw failure("helper-unavailable", "not-attempted", result.error);
  }
  ...
  if (response.code === "EEXIST" || response.code === "ENOTEMPTY") throw failure("already-exists", "not-attempted", cause);
