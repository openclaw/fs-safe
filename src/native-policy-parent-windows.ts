...
function closeAfterFailure(fd: number | undefined): void {
  ...
  try {
    for (let index = 0; index < segments.length; index += 1) {
      ...
      try {
        ...
        closeAfterFailure(child.fd);
        throw error;
      }
      const previousFd = ownedFd;
      current = child;
      ownedFd = child.fd;
      if (previousFd !== undefined) fsSync.closeSync(previousFd);
    }
