...
import { WINDOWS_SECURITY_SOURCE } from "../src/windows-security-source.js";
...
describe.runIf(process.platform === "win32")("Windows built-in security commands without addons", () => {
  ...
  it.each([
    ...
    let source = replaceOnce(WINDOWS_SECURITY_SOURCE,
      ...
      "public static class FsSafeWindowsBridge { public static int LocalityCalls, CreateCalls, IdentityQueries;");
    ...
    const stdout = execFileSync(resolveWindowsSystemCommand(String.raw`WindowsPowerShell\v1.0\powershell.exe`), [
      ...
      env: { ...process.env, FS_SAFE_LOCALITY_FAIL_AT: String(failAt), FS_SAFE_LOCALITY_TARGET: target },
    });
  ...
  it("matches the native locality spelling rules for volume, DOS, and UNC paths", () => {
    ...
    const stdout = execFileSync(resolveWindowsSystemCommand(String.raw`WindowsPowerShell\v1.0\powershell.exe`), [
      ...
      input: WINDOWS_SECURITY_SOURCE, encoding: "utf8", windowsHide: true, timeout: 30_000,
      env: { ...process.env, FS_SAFE_WINDOWS_LOCALITY_CASES: JSON.stringify(cases) },
    });
  ...
  it("allows missing-capability fallbacks in require mode after a successful addon load", async () => {
    configureFsSafeNative({ mode: "require" });
