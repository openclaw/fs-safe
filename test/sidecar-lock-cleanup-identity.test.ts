...
function projectIdentity(stat: Stats, identity: { dev?: number; ino?: number }): Stats {
  return Object.assign(Object.create(Object.getPrototypeOf(stat)), stat, identity) as Stats;
}
...
describe("failed sidecar creation with unknown Windows identity", () => {
  ...
  it.each([false, true])(
    "preserves the sync pathname and original error while closing both handles (lockRoot: %s)",
    async (withLockRoot) => {
      const directory = await tempRoot("fs-safe-sidecar-cleanup-sync-unknown-");
      const capability = withLockRoot ? await root(directory) : undefined;
      const targetPath = path.join(directory, "state.json");
      const lockPath = `${targetPath}.lock`;
      const displacedPath = `${lockPath}.failed-create`;
      const replacementPath = `${lockPath}.replacement`;
