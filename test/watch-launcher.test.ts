...
it("supports the runtime ESM launcher without clearing inherited flags", async () => {
  ...
  const script = [
    ...
    "try { owner=watch(await root(directory),{mode:" + JSON.stringify(nativeWatchSupported ? "node" : "poll") + ",scopes:[{path:'',kind:'tree'}],onDirty(){}}); await owner.ready; console.log('ready'); }",
