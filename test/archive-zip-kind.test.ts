...
const runFixture = useSuiteFixture(async () => {
  suiteDir = await fs.mkdtemp(path.join(os.tmpdir(), "fs-safe-zip-kind-suite-"));
...
function run(operation: (directory: string) => Promise<void>) {
  return runFixture(async (directory) => {
    try { await operation(directory); }
      ...
      // Restore backend selection and mocks only after pending I/O settles.
      vi.restoreAllMocks();
...
async function fixture(directory: string, entries: ZipRecord[]) {
  const dir = await fs.mkdtemp(path.join(directory, "case-"));
...
for (const mode of ["off", "require"] as const) {
  describe.skipIf(mode === "require" && !native)(`ZIP kind contract (${mode})`, () => {
    ...
    for (const creatorSystem of [0, 3, 19, 255]) {
      it.each(directories)(`classifies $label for creator ${creatorSystem}`, ({ name, attributes }) => run(async (directory) => {
        configure();
        const input = await fixture(directory, [{ name, attributes, creatorSystem, body: "" }]);
      ...
      it.each(["item", "item/", "item\\"])(`keeps high-word symlinks first for creator ${creatorSystem}: %s`, name => run(async (directory) => {
        configure();
        const input = await fixture(directory, [{ name, creatorSystem, attributes: 0xa1ff0010, body: "target" }]);
        const filter = vi.fn(() => "skip" as const);
        await extractArchive({ ...input, entryFilter: filter, onFiltered: "skip-entry" });
    ...
    ])("rejects marker disagreement before any callback: %j", entry => run(async (directory) => {
      configure();
      const input = await fixture(directory, [{ name: "keep" }, { ...entry, attributes: 0x10 }]);
      const filter = vi.fn(() => "skip" as const);
    ...
    it("preserves internal separator/dot aliases and matching terminal markers", () => run(async (directory) => {
      configure();
    ...
    it("does not mistake a UNIX socket type for the exact directory type", () => run(async (directory) => {
      configure();
      const input = await fixture(directory, [{ name: "item", creatorSystem: 3, attributes: 0xc1ed0000 }]);
      const filter = vi.fn(() => "skip" as const);
      await extractArchive({ ...input, entryFilter: filter, onFiltered: "skip-entry" });
