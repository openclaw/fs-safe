...
describe("manual split-credential secure-file proof contract", () => {
  ...
  it("runs the reviewed harness and authenticated Node from the exact protected stage", async () => {
    ...
    expect(proof).toContain("PROOF_PARENT: /usr/local/lib/fs-safe-credential-proof-");
    expect(proof).toContain("/fs-safe-secure-file-proof-${{ matrix.node }}");
    ...
    expect(coordinator).toContain("proofParentPath !== path.join(expectedStageDirectory");
    expect(coordinator).toContain("proofParentContainerStat.uid !== 0n");
    expect(coordinator).toContain("receiptContainerCanonical !== receiptContainer");
    expect(coordinator).toContain("receiptContainerStat.uid !== BigInt(builderUid)");
