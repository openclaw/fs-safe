...
function forceSplitCredentialProbe(): number {
  const effectiveUid = process.geteuid!();
  const effectiveGid = process.getegid!();
  const differentRealUid = effectiveUid === 0 ? 1 : effectiveUid - 1;
  vi.spyOn(process, "getuid").mockReturnValue(differentRealUid);
...
describe("resolveSecureTempRoot", () => {
  ...
  it("does not treat an access-time ENOENT as a missing directory", () => {
    const preferredDir = "/virtual/preferred";
    const fallbackPath = path.join("/virtual/fallback", "example-501");
    const accessSync = vi.fn((candidate: string) => {
    ...
    expect(resolveSecureTempRoot({
      ...
      platform: "linux",
      preferredDir,
      tmpdir: () => "/virtual/fallback",
    })).toBe(fallbackPath);
    expect(mkdirSync).not.toHaveBeenCalled();
    expect(chmodSync).not.toHaveBeenCalled();
  });

  itPosix("retains the access check for matching non-root process credentials", () => {
    vi.spyOn(process, "getuid").mockReturnValue(501);
    vi.spyOn(process, "geteuid").mockReturnValue(501);
    vi.spyOn(process, "getgid").mockReturnValue(20);
    vi.spyOn(process, "getegid").mockReturnValue(20);
    const accessSync = vi.spyOn(fs, "accessSync").mockImplementation(() => undefined);
    const openSync = vi.spyOn(fs, "openSync");

    expect(resolveSecureTempRoot({
      fallbackPrefix: "example",
      lstatSync: vi.fn(() => secureDirStat(501)),
      platform: process.platform,
      preferredDir: "/virtual/preferred",
    })).toBe("/virtual/preferred");
    expect(accessSync).toHaveBeenCalledTimes(1);
    expect(openSync).not.toHaveBeenCalled();
  });

  itPosix("treats operation-probe ENOENT as invalid without creating or repairing", () => {
    ...
    const probeLstat = vi.spyOn(fs, "lstatSync").mockImplementation((() => {
      throw nodeErrorWithCode("ENOENT");
    ...
    expect(() => resolveSecureTempRoot({
      ...
      fallbackPrefix: "example",
      getuid: () => 0,
      lstatSync: vi.fn(() => secureDirStat(0)),
      mkdirSync,
      platform: process.platform,
      tmpdir: () => "/virtual/fallback",
    })).toThrow("Unsafe fallback secure temp dir");
    expect(probeLstat).toHaveBeenCalled();
    expect(accessSync).not.toHaveBeenCalled();
    expect(mkdirSync).not.toHaveBeenCalled();
    expect(chmodSync).not.toHaveBeenCalled();
  });

  itPosix("proves split-credential access and removes the received probe file", () => {
    const effectiveUid = forceSplitCredentialProbe();
    const base = createTemporaryDirectory("effective-access");
  ...
  itPosix("uses the operation probe when only the process group identities differ", () => {
    const effectiveUid = process.geteuid!();
    const effectiveGid = process.getegid!();
    vi.spyOn(process, "getuid").mockReturnValue(effectiveUid);
    vi.spyOn(process, "geteuid").mockReturnValue(effectiveUid);
    vi.spyOn(process, "getgid").mockReturnValue(
      effectiveGid === 0 ? 1 : effectiveGid - 1,
    );
    vi.spyOn(process, "getegid").mockReturnValue(effectiveGid);
    const base = createTemporaryDirectory("effective-group");
    const preferredDir = path.join(base, "preferred");
    fs.mkdirSync(preferredDir, { mode: 0o700 });
    fs.chmodSync(preferredDir, 0o700);
    const accessSync = vi.spyOn(fs, "accessSync");
  ...
  itPosix("creates and probes a missing preferred directory under split credentials", () => {
    forceSplitCredentialProbe();
    const base = createTemporaryDirectory("effective-create");
    const preferredDir = path.join(base, "preferred");
    const accessSync = vi.spyOn(fs, "accessSync");
  ...
  itPosixNonRoot("probes the admitted directory rather than a normalized symlink-dotdot path", () => {
    forceSplitCredentialProbe();
    const base = createTemporaryDirectory("effective-dotdot");
    ...
    fs.chmodSync(canonicalParent, 0o500);
    ...
    try {
      ...
      expect(fs.readdirSync(expectedFallback)).toEqual([]);
    } finally {
      fs.chmodSync(canonicalParent, 0o700);
    }
  });
