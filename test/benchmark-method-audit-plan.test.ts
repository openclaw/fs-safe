...
function rawReport(plan: ReturnType<typeof planFor>, reportPlan = plan.reports[0]) {
  ...
  return {
    ...
    metadata: {
      ...
      distributionIdentity: distributionIdentityForReport(plan, reportPlan),
      measuredProfiles: { filenameFallbackSanitization: "sanitized" },
      nativeHash: null,
      node: "v24.9.0",
      platform: "win32",
      arch: "x64",
...
describe("method-audit sequence and build controls", () => {
  ...
  it("maps same-artifact labels to one build and one candidate dist path", () => {
    ...
    expect(distributionIdentityForReport(plan, baselinePlan)).toBe("candidate-equivalent");
  });
  ...
  it("keeps spaces and a leading slash filter as literal argv entries", () => {
    ...
    const args = createRunnerArguments({
      ...
      distributionIdentity: "candidate-equivalent",
    ...
    expect(args).toContain("candidate-equivalent");
  });

  it("maps distinct H/C/B checkouts to the measured C/B distributions", () => {
    const plan = planFor();
    const candidatePlan = plan.reports.find(({ role }) => role === "candidate")!;
    const baselinePlan = plan.reports.find(({ role }) => role === "baseline")!;
    expect([plan.harness.sha, plan.sources.candidate.commit, plan.sources.baseline?.commit])
      .toEqual([H, C, B]);
    expect(distributionIdentityForReport(plan, candidatePlan)).toBe("candidate-equivalent");
    expect(distributionIdentityForReport(plan, baselinePlan)).toBe("comparison");
  });
});
...
describe("method-audit provenance validation", () => {
  ...
  it("fails incomplete, mutated, distribution-mismatched, and native-mismatched evidence", () => {
    ...
    const wrongIdentity = structuredClone(reports);
    wrongIdentity.get(plan.reports.find(({ role }) => role === "candidate")!.file)!
      .metadata.distributionIdentity = "comparison";
    expect(() => validateCompleteReportSet(plan, wrongIdentity, before, before))
      .toThrow("distribution identity mismatch");

    const wrongProfile = structuredClone(reports);
    wrongProfile.get(plan.reports.find(({ role }) => role === "candidate")!.file)!
      .metadata.measuredProfiles.filenameFallbackSanitization = "legacy";
    expect(() => validateCompleteReportSet(plan, wrongProfile, before, before))
      .toThrow("candidate-equivalent filename fallback profile mismatch");
...
describe("benchmark workflow contract", () => {
  ...
  it("measures C/B without requiring a built H distribution", async () => {
    const evidenceDriver = await readFile("benchmarks/method-audit-evidence.mjs", "utf8");
    const runner = await readFile("benchmarks/runner.mjs", "utf8");
    expect(evidenceDriver).toContain('distRoot: path.join(roots[build.checkout], "dist")');
    expect(evidenceDriver).toContain("distributionIdentity: distributionIdentityForReport(plan, reportPlan)");
    expect(runner).toContain('const dist = path.resolve(args.dist ?? localDist)');
    expect(runner).not.toContain("candidateDistHash");
    expect(runner).not.toContain('distributionHash(path.join(packageRoot, "dist"))');
  });
