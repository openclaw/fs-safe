...
import { __setNativeLoaderForTest } from "../src/native.js";
...
describe("retained directory replacement authority", () => {
    ...
    async ({ distinctParents, existingTarget }) => {
      ...
      await expect(replaceDirectoryAtomic({ stagedDir: setup.staged, targetDir: setup.target }))
        .rejects.toMatchObject({ details: {
          phase: "publish", publication: "indeterminate",
          recovery: existingTarget ? "backup-path-preserved" : "none",
        } });
      expect(setup.renameNoReplace).toHaveBeenCalledTimes(existingTarget ? 2 : 1);
  ...
  it("rolls back through no-replace after a definite publication failure", async () => {
    ...
    setup.hooks.beforeRename = ({ sourceName }) => {
      if (sourceName === "staged") {
        throw Object.assign(new Error("source fence rejected before dispatch"), {
  ...
  it.each(
    ["backup", "publish", "rollback", "publish-absent"].flatMap(phase =>
      ["ENOENT", "EEXIST", "EACCES", "EIO"].map(code => ({ phase, code }))),
  )("preserves a committed $phase rename after $code", async ({ phase, code }) => {
    const setup = await fixture({ existingTarget: phase !== "publish-absent" });
    setup.hooks.beforeRename = ({ sourceName }) => {
      if (phase === "rollback" && sourceName === "staged") {
        throw Object.assign(new Error("source fence rejected before dispatch"), {
          code: NATIVE_RENAME_SOURCE_IDENTITY_MISMATCH,
    ...
    setup.hooks.afterRename = ({ call, sourceName }) => {
      if (phase === "backup" && call === 1 ||
          phase.startsWith("publish") && sourceName === "staged" ||
          phase === "rollback" && call === 3) {
        throw Object.assign(new Error("remote acknowledgement lost"), { code });
      }
    };

    const error = await replaceDirectoryAtomic({ stagedDir: setup.staged, targetDir: setup.target })
      .catch((error: unknown) => error);
    const published = phase.startsWith("publish");
    expect(error).toMatchObject({ details: {
      phase: published ? "publish" : phase,
      publication: published ? "indeterminate" : "not-published",
      recovery: phase === "publish" ? "backup-path-preserved"
        : phase === "publish-absent" ? "none" : "indeterminate",
    } });
    expect(setup.renameNoReplace).toHaveBeenCalledTimes(
      phase === "publish" ? 2 : phase === "rollback" ? 3 : 1,
    );
    expect(setup.removeOwnedTree).not.toHaveBeenCalled();
    if (phase === "backup") {
      await expect(fs.lstat(setup.target)).rejects.toMatchObject({ code: "ENOENT" });
    } else {
      expect(await fs.readFile(path.join(setup.target, "value.txt"), "utf8"))
        .toBe(published ? "new" : "old");
    }
    if (!published) {
      expect(await fs.readFile(path.join(setup.staged, "value.txt"), "utf8")).toBe("new");
    }
    const backups = await backupPaths(setup.targetParent);
    expect(backups).toHaveLength(phase === "backup" || phase === "publish" ? 1 : 0);
    if (backups[0]) {
      expect(error).toMatchObject({ details: { backupPath: backups[0] } });
      expect(await fs.readFile(path.join(backups[0], "value.txt"), "utf8")).toBe("old");
    } else if (phase === "rollback") {
      expect(error).toMatchObject({ details: { backupPath: expect.any(String) } });
    }
  });
  ...
  it("preserves the primary failure and recovery details when descriptor close also fails", async () => {
    ...
    await expect(replaceDirectoryAtomic({ stagedDir: setup.staged, targetDir: setup.target }))
      .rejects.toMatchObject({
        code: "EACCES",
        details: { publication: "indeterminate", recovery: "backup-path-preserved" },
      });

    expect(injected).toBe(true);
    const [backup] = await backupPaths(setup.targetParent);
    expect(backup).toBeDefined();
    await expect(fs.readFile(path.join(backup!, "value.txt"), "utf8")).resolves.toBe("old");
  });
